...
Simulation de différents scénarios et comparaison de leur impact sur la capacité hospitalière.
"""
...
preset_labels = {
    "Custom": "Personnalisé",
    "Baseline": "Référence",
    "Mild Epidemic": "Épidémie Légère",
    "Severe Epidemic": "Épidémie Sévère",
    "Staff Strike": "Grève du Personnel",
    "Winter Peak": "Pic Hivernal",
    "Summer Heatwave": "Canicule Estivale",
...
if selected_preset != "Custom":
    ...
    default_epidemic = preset.epidemic_intensity
    default_staffing = preset.staffing_reduction
    default_seasonal = preset.seasonal_multiplier
    default_shock = preset.shock_day_spike
    default_beds = preset.beds_reduction
    default_stock = preset.stock_reduction
else:
...
with col1:
    ...
    st.metric(
        "Admissions Moy./Jour",
        f"{scenario_summary['avg_daily_admissions']:.0f}",
...
with col2:
    st.metric(
        "Jour de Pointe",
        f"{scenario_summary['max_daily_admissions']:.0f}",
        delta=f"{scenario_summary['max_daily_admissions'] - baseline_summary['max_daily']:.0f}"
    )

with col3:
    st.metric(
        "Jours en Surcapacité",
        f"{scenario_summary['days_overcapacity']:.0f}",
        delta=None,
        delta_color="inverse"
    )
...
with col4:
    st.metric(
        "Occupation Moy.",
        f"{scenario_summary['avg_occupancy_rate']:.0%}",
        delta=f"{(scenario_summary['avg_occupancy_rate'] - 0.45)*100:+.0f}pp vs référence"
    )
...
with tab1:
    ...
    fig.add_trace(go.Scatter(
        x=baseline_df["date"],
        y=baseline_df["predicted_admissions"],
        mode="lines",
        name="Prévision de Référence",
    ...
    fig.add_trace(go.Scatter(
        x=scenario_df["date"],
        y=scenario_df["scenario_admissions"],
        mode="lines",
        name="Prévision du Scénario",
        line=dict(color="#ff7f0e", width=2)
    ))

    # Capacity line
    fig.add_hline(
        y=CAPACITY_CONFIG.normal_admission_capacity,
        line_dash="dash",
        line_color="red",
        annotation_text=f"Capacité ({CAPACITY_CONFIG.normal_admission_capacity})"
    )

    # Effective capacity (if reduced)
    if scenario_df["effective_capacity"].mean() < CAPACITY_CONFIG.normal_admission_capacity:
        fig.add_trace(go.Scatter(
            x=scenario_df["date"],
            y=scenario_df["effective_capacity"],
            mode="lines",
            name="Capacité Effective",
    ...
    fig.update_layout(
        ...
        xaxis_title="Date",
        yaxis_title="Admissions Quotidiennes",
        hovermode="x unified",
        height=500,
    ...
    fig_delta.add_trace(go.Bar(
        x=scenario_df["date"],
        y=scenario_df["demand_change_pct"],
        marker_color=np.where(scenario_df["demand_change_pct"] > 0, "#ff7f0e", "#1f77b4"),
        name="Variation %"
    ))

    fig_delta.update_layout(
        title="Variation en Pourcentage des Admissions",
        xaxis_title="Date",
...
with tab2:
    ...
    with col1:
        ...
        fig_gap.add_trace(go.Bar(
            x=scenario_df["date"],
            y=scenario_df["capacity_gap"],
            marker_color=colors,
            name="Écart de Capacité"
        ))

        fig_gap.add_hline(y=0, line_color="black", line_width=1)

        fig_gap.update_layout(
            title="Écart de Capacité Quotidien (Demande - Capacité)",
            xaxis_title="Date",
            yaxis_title="Écart (patients)",
            height=400
        )
    ...
    with col2:
        ...
        fig_occ.add_trace(go.Scatter(
            x=scenario_df["date"],
            y=scenario_df["occupancy_rate"] * 100,
            mode="lines+markers",
        ...
        fig_occ.add_hline(
            y=CAPACITY_CONFIG.critical_occupancy_threshold * 100,
            line_dash="dash",
            line_color="red",
            annotation_text="Critique (85%)"
        )

        fig_occ.add_hline(
            y=CAPACITY_CONFIG.warning_occupancy_threshold * 100,
            line_dash="dash",
            line_color="orange",
            annotation_text="Alerte (75%)"
        )
...
with tab3:
    ...
    detail_df.columns = [
        "Date", "Référence", "Scénario", "Capacité", "Écart",
        "Occupation", "Surcapacité", "Critique"
    ]
    ...
    st.download_button(
        ...
        file_name=f"scenario_{selected_preset.lower().replace(' ', '_')}.csv",
        mime="text/csv"
    )
...
with st.expander("Comparer Plusieurs Scénarios"):
    ...
    for name, preset in preset_scenarios.items():
        ...
        comparison_data.append({
            "Scénario": preset_labels.get(name, name),
            "Admissions Moy.": summary["avg_daily_admissions"],
            "Jour de Pointe": summary["max_daily_admissions"],
            "Jours Surcapacité": summary["days_overcapacity"],
            "Jours Critiques": summary["days_critical"],
            "Écart Max": summary["max_capacity_gap"],
